...
def plot_metric_individual(models_path, prefix, query_run, loader_name, metric_key, metric_name, metrics_genome_dict, metrics_nogenome_dict, plt_dir):
    ...
    counts_sum = np.clip(np.log(counts_to + epsilon) + np.log(counts_from + epsilon), 10, 15)
    ...
    df_genome = pd.DataFrame(arr_genome, columns=["Coordinates", "Enrichment Difference", "Enrichment Sum", name_genome])
