...
def plot_metric_individual(models_path, prefix, query_run, loader_name, metric_key, metric_name, metrics_genome_dict, metrics_nogenome_dict, plt_dir):
    ...
    sns.scatterplot(data=df_merged, x="Enrichment Differential", y=name_genome, hue="Log10 Total Counts", s=6, hue_norm=(1, 5))
    plt.title(f"{name_genome} vs. Peak Specificity")
    plt_path = os.path.join(plt_dir, f"individual_genome.svg")
    plt.savefig(plt_path, bbox_inches='tight')
    plt.clf()

    sns.scatterplot(data=df_merged, x="Enrichment Differential", y=name_nogenome, hue="Log10 Total Counts", s=6, hue_norm=(1, 5))
    plt.title(f"{name_nogenome} vs. Peak Specificity")
    plt_path = os.path.join(plt_dir, f"individual_nogenome.svg")
    plt.savefig(plt_path, bbox_inches='tight')
    plt.clf()

    sns.scatterplot(data=df_merged, x="Enrichment Differential", y=name_diff, hue="Log10 Total Counts", s=6, hue_norm=(1, 5))
    ...
    sns.scatterplot(data=df_merged, x=name_nogenome, y=name_genome, hue="Log10 Total Counts", s=6, hue_norm=(1, 5))
