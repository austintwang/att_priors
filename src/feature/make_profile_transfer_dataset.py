...
class SamplingCoordsBatcher(torch.utils.data.sampler.Sampler):
    ...
    def __init__(
        ...
        if peaks_trans_thresh_type is None:
            ...
            for pos_coords_bed_trans in pos_coords_beds_trans:
                ...
                selects = peak_table_trans
                chroms = selects["chrom"]
                starts = selects["peak_start"]
                ends = selects["peak_end"]
    ...
    @staticmethod
    def _query_peak(query_table, peak, thresh_type):
        ...
        for c in candidates:
            ...
            if start <= end_c and start_c <= end:
                return True == return_sig

        # print(candidates) ####

        return False == return_sig
...
class CoordDatasetTrans(CoordDataset):
    def __init__(
        self, coords_batcher, coords_to_seq, coords_to_vals, coords_to_vals_trans, revcomp=False,
        return_coords=False
    ):
        super().__init__(coords_batcher, coords_to_seq, coords_to_vals, revcomp, return_coords)
        self.coords_to_vals_trans = coords_to_vals_trans
    ...
    def get_batch(self, index):
        ...
        # Map this batch of coordinates to the associated values
        profiles = self.coords_to_vals(coords)
        profiles_trans = self.coords_to_vals_trans(coords)

        # Profiles are returned as B x O x P x S, so transpose to get
        # B x P x O x S
        profiles = np.swapaxes(profiles, 1, 2)
        profiles_trans = np.swapaxes(profiles_trans, 1, 2)
        ...
        if self.revcomp:
            ...
            profiles_trans = np.concatenate(
                [profiles_trans, np.flip(profiles_trans, axis=(2, 3))]
            )
            status = np.concatenate([status, status])
        ...
        if self.return_coords:
            if self.revcomp:
                ...
                coords_ret, peaks_ret = coords, peaks
            return seqs, profiles, profiles_trans, status, coords_ret, peaks_ret
        else:
            return seqs, profiles, profiles_trans, status
...
def create_data_loader(
    ...
    coords_to_vals_trans = CoordsToVals(profile_trans_hdf5_path, profile_length)
    ...
    #     )
        ...
        # Yields batches of positive coordinates, centered at summits
        coords_batcher = SummitCenteringCoordsBatcher(
            peak_bed_trans_paths, peaks_bed_paths, batch_size, chrom_sizes_tsv, input_length,
            chroms_keep=chrom_set, return_peaks=return_coords,
            shuffle_before_epoch=shuffle, shuffle_seed=shuffle_seed
        )
    ...
    # Dataset
    dataset = CoordDatasetTrans(
        coords_batcher, coords_to_seq, coords_to_vals, coords_to_vals_trans, revcomp=revcomp,
