...
class ProfileTFBindingPredictor(torch.nn.Module):
    ...
    def __init__(
        ...
        self.prof_large_conv = torch.nn.Conv1d(
            in_channels=dil_conv_depths[-1], out_channels=num_tasks,
        ...
        self.prof_one_conv = torch.nn.Conv1d(
            in_channels=num_tasks, out_channels=num_tasks,
        ...
        self.count_dense = torch.nn.Linear(
            in_features=dil_conv_depths[-1], out_features=num_tasks
        )
    ...
    def forward(self, input_seqs):
        ...
        Returns the predicted profiles (unnormalized logits) for each task
        (a B x T x O tensor), and the predicted log counts (base e) for each
        task (a B x T) tensor.
        """
        ...
        # controls; there are T convolutions, each one is done over only a
        # single entry
        prof_pred = self.prof_one_conv(prof_large_conv_out)
        ...
        count_dense_out = self.count_dense(count_pool_out)  # Shape: B x T
        count_dense_out = count_dense_out.view(batch_size, self.num_tasks, 1)
        # Shape: B x T x 1
        ...
        # Shape: B x T x 1
        count_pred = torch.squeeze(count_one_conv_out, dim=2)  # Shape: B x T
    ...
    def correctness_loss(
        ...
        Arguments:
            `true_profs`: a B x T x O tensor containing true UNnormalized
            ...
            `logit_pred_profs`: a B x T x O tensor containing the predicted
                profile _logits_
            `log_pred_counts`: a B x T tensor containing the predicted log
                read counts (base e)
        ...
        true_counts = torch.sum(true_profs, dim=2)  # Shape: B x T
