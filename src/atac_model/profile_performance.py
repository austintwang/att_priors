...
def profile_multinomial_nll(true_profs, log_pred_profs, true_counts):
    ...
    across the strands.
    Arguments:
        `true_profs`: N x T x O array, where N is the number of
        ...
        `log_pred_profs`: a N x T x O array, containing the predicted
            profiles for each task and strand, as LOG probabilities
        `true_counts`: a N x T array, containing the true total counts
    ...
    return -multinomial_log_probs(log_pred_profs, true_counts, true_profs)
...
def profile_jsd(true_prof_probs, pred_prof_probs, jsd_smooth_kernel_sigma):
    ...
    is sufficient.
    Arguments:
        `true_prof_probs`: N x T x O array, where N is the number of
        ...
        `pred_prof_probs`: N x T x O array, containing the predicted
    ...
    true_prof_smooth = scipy.ndimage.gaussian_filter1d(
        true_prof_probs, sigma, axis=-1, truncate=truncate
    )
    pred_prof_smooth = scipy.ndimage.gaussian_filter1d(
        pred_prof_probs, sigma, axis=-1, truncate=truncate
    )

    return jensen_shannon_distance(true_prof_smooth, pred_prof_smooth)
...
def binned_profile_auprc(
    ...
    Arguments:
        `true_prof_counts`: B x T x O array, where B is the batch size, T is
        ...
        `pred_prof_probs`: a B x T x O array, containing the predicted
            profiles for each task and strand, as RAW PROBABILITIES
        `true_total_counts`: a B x T array, containing the true total counts
            for each task and strand
    ...
    for i, bin_size in enumerate(auprc_bin_sizes):
        ...
        true_prob_bins = bin_array_max(true_prof_probs, bin_size)
        true_count_bins = bin_array_max(true_prof_counts, bin_size)
        pred_prob_bins = bin_array_max(pred_prof_probs, bin_size)
...
def binned_profile_corr_mse(
    ...
    per base or per bin).
    Arguments:
        `true_prof_probs`: a N x T x O array, containing the true profile
            RAW PROBABILITIES for each task and strand
        `pred_prof_probs`: a N x T x O array, containing the true profile
            RAW PROBABILITIES for each task and strand
        `batch_size`: performs computation in a batch size of this many samples
    ...
    for i, bin_size in enumerate(prof_count_corr_bin_sizes):
        true_prob_binned = bin_array_max(true_prof_probs, bin_size)
        pred_prob_binned = bin_array_max(pred_prof_probs, bin_size)
        for start in range(0, num_samples, batch_size):
            end = start + batch_size
...
def total_count_corr_mse(log_true_total_counts, log_pred_total_counts):
    ...
    Arguments:
        `log_true_total_counts`: a N x T array, containing the true total
            LOG COUNTS for each task and strand
        `log_pred_prof_counts`: a N x T array, containing the predicted
    ...
    strands.
    """
    # N x T -> T x N; correlations taken over N
    log_true_total_counts = np.swapaxes(log_true_total_counts, 0, 1)
    log_pred_total_counts = np.swapaxes(log_pred_total_counts, 0, 1)
...
def compute_performance_metrics(
    ...
    Arguments:
        ...
        `true_counts`: a N x T array, containing the true total counts
            for each task and strand
        `log_pred_counts`: a N x T array, containing the predicted LOG total
    ...
    Returns 2 dictionaries. The first dictionary contains:
        ...
        A N x T x Z x 4 array of the auPRCs for the binned and binarized
            ...
            is reported for Z bin sizes, for each sample/task
