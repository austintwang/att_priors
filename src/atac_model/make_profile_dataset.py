...
class CoordsToVals:
    ...
    Arguments:
        `hdf5_path`: path to HDF5 containing profiles; this HDF5 must have a
            ...
            profiles of shape L x S, where L is the coordinate dimension,
            S is the number of profile tracks
    ...
    def _get_ndarray(self, coords):
        ...
        This will be a 3D NumPy array of corresponding profile values. If a
...
class CoordDataset(torch.utils.data.IterableDataset):
    ...
    def get_batch(self, index):
        ...
        # Profiles are returned as B x L x T, so transpose to get B x T x L
        ...
        if self.revcomp:
            profiles = np.concatenate(
                [profiles, np.flip(profiles, axis=2)]
            )
