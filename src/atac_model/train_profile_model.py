...
def model_loss(
    ...
    Arguments:
        ...
        `true_profs`: a B x T x O tensor, where B is the batch size, T is
            the number of tasks, and O is the length of the output profiles;
            this contains true profile read counts (unnormalized)
        `log_pred_profs`: a B x T x O tensor, consisting of the output
            profile predictions as logits
        `log_pred_counts`: a B x T tensor consisting of the log counts
            predictions
...
def run_epoch(
    ...
    if return_data:
        ...
        profile_shape = (num_samples_exp, num_tasks, profile_length)
        count_shape = (num_samples_exp, num_tasks)
    ...
    for data_batch in t_iter:
        ...
        loss, (corr_loss, att_loss), (prof_loss, count_loss) = model_loss(
            model, profiles, logit_pred_profs, log_pred_counts, epoch_num,
        ...
        if return_data:
            ...
            true_profs_np = profiles.detach().cpu().numpy()
